...
python3 ../jobs_launcher/executeTests.py --file_filter $FILE_FILTER --test_filter $TESTS_FILTER --tests_root ../jobs --work_root ../Work/Results --work_dir Maya --cmd_variables Tool "/Applications/Autodesk/maya${TOOL}/Maya.app/Contents/bin" RenderDevice "$RENDER_DEVICE" ResPath "$CIS_TOOLS/../TestResources/rpr_maya_autotests_assets" PassLimit $ITER rx $RX ry $RY SPU $SPU threshold $THRESHOLD engine $ENGINE retries $RETRIES UpdateRefs $UPDATE_REFS
